...
######################### no replacement ########################   
    [string]$global:MessageLogFilePath          = "$ProjectPath\$DATAFolder\ExchangeMessageLogs.csv"                 # Path to message log file.
    [string]$global:AgentLogFilePath            = "$ProjectPath\$DATAFolder\ExchangeAgentLogs.csv"                   # Path to agent log file.
    [string]$global:MessageLogFilePath1         = "$ProjectPath\$DATAFolder\ExchangeMessageLogsTrm.csv"              # Path to transformed message log file.
    [string]$global:ActiveSyncLogFilePath       = "$ProjectPath\$DATAFolder\ActiveSyncLog.csv"                       # Path to active sync log file.
    [string]$global:AgentLogFilePath1           = "$ProjectPath\$DATAFolder\ExchangeAgentLogsTrm.csv"                # Path to agent transformed log file.
    [string]$global:FilterConfig                = "$ProjectPath\$DATAFolder\FilterConfig.xml"                        # Path to filter config XML file.
    [string]$global:FilterConfigPath1           = "$ProjectPath\$DATAFolder\FilterConfig.csv"                        # Path to filter config CSV file.
